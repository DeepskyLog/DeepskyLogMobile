...
/**
 * The UUIDGenerator implements the IIDGenerator
 * interface by using java.util.UUID, which creates
 * universal unique IDs. (Requires JRE 1.5 or higher)
 * 
...
public class UUIDGenerator implements IIDGenerator {
	...
	public UUIDGenerator() throws Exception {
		...
		// This will cause the ClassNotFoundException in case the UUID class cannot be found (VM Version < 1.5)
	...
	// -------------------------------------------------------------------
	/**
	 * Creates a universal unique ID that can be used to identify and link
	 * several schema elements.<br>
	 * All elements inside a XML file need to have a unique ID.
	 * 
	 * @return Returns a universal unique ID that can be used to identify 
