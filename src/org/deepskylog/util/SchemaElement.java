...
import org.deepskylog.util.util.IIDGenerator;
...
public abstract class SchemaElement implements ISchemaElement {
	...
	static {
		...
		if( after14 ) {   	// Use UUID
			try {
				IDGenerator = new org.deepskylog.util.util.UUIDGenerator();				
			} catch( Exception e ) {				
				System.err.println("Cannot create UUID Generator. Try to get UIDGenerator...");
				IDGenerator = new org.deepskylog.util.util.UIDGenerator();
			}
		} else {      		// Use UID
			IDGenerator = new org.deepskylog.util.util.UIDGenerator();
		}
