...
public class EquPosition extends SchemaElement {
   ...
   * Checks if this EquPosition and the given Object are equal. The given object is equal with this EquPosition, if the
   * right ascension, declination and the position reference frame are equal.
   * 
   * @param obj
  ...
  // -------------------------------------------------------------------
  /**
   * Adds this EquPosition to an given parent XML DOM Element. The EquPosition Element will be set as a child element of
   * the passed Element.
   * 
   * @param parent
  ...
  public double getRaDecimalHours() {
    ...
    ra = ra + (Integer.parseInt(s.substring(s.indexOf(EquPosition.RA_HOUR) + 1, s.indexOf(EquPosition.RA_MIN)))) / 60.0;
  ...
  public void setDec(String dec) throws IllegalArgumentException {
    ...
    try {
      ...
      int min = Integer.parseInt(dec.substring(dec.indexOf(EquPosition.DEC_DEG) + 1, dec.indexOf(EquPosition.DEC_MIN)));
      double sec = Double.parseDouble(dec.substring(dec.indexOf(EquPosition.DEC_MIN) + 1,
  ...
  public void setRa(String ra) throws IllegalArgumentException {
    ...
    try {
      ...
      int min = Integer.parseInt(ra.substring(ra.indexOf(EquPosition.RA_HOUR) + 1, ra.indexOf(EquPosition.RA_MIN)));
      double sec = Double.parseDouble(ra.substring(ra.indexOf(EquPosition.RA_MIN) + 1, ra.indexOf(EquPosition.RA_SEC)));

      double r = (hour * 15) + ((double) min / 60.0 * 15) + ((double) sec / 3600.0 * 15);
